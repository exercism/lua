...
describe('high-scores', function()
  ...
  local SameValuesInOrder

  setup(function()
    SameValuesInOrder = function(a, b)
      if #a ~= #b then
        return false
      end
  ...
  teardown(function()
    SameValuesInOrder = nil
  end)

  it('Table of scores', function()
    local values = {30, 50, 20, 70}
    local highScores = HighScores{values}
    local expected = {30, 50, 20, 70}
    assert.True(SameValuesInOrder(expected, highScores:Scores()))
  end)
  ...
  it('Latest score', function()
    ...
    local highScores = HighScores{values}
    local expected = 30
    assert.equal(expected, highScores:Latest())
  end)
  ...
  it('Personal best', function()
    ...
    local highScores = HighScores{values}
    local expected = 100
    assert.equal(expected, highScores:PersonalBest())
  end)
  ...
  it('Personal top three from a list of scores', function()
    ...
    assert.True(SameValuesInOrder(expected, highScores:PersonalTopThree()))
  end)
  ...
  it('Latest score after personal top scores', function()
    ...
    local highScores = HighScores{values}
    local _ = highScores:PersonalTopThree()
    local expected = 30
    assert.equal(expected, highScores:Latest())
  end)
  ...
  it('Latest score after personal best', function()
    ...
    local highScores = HighScores{values}
    local _ = highScores:PersonalBest()
    local expected = 30
    assert.equal(expected, highScores:Latest())
  end)
